...
namespace Bl
{
    public interface IItems
    {
        ...
        public List<TbItem> GetByItemId(int id);
        public List<VwItem> GetByCatId(int id);
    ...
    public class ClsItems : IItems
    {
        ...
        public ClsItems(DbStoreContext ctx)
        {
            context = ctx;
        }
        public List<TbItem> GetAll()
        ...
        public List<VwItem> GetRecommendedItems(int itemId)
        {
            try
            {
                ...
                var lstCategories = context.VwItems.Where(a => a.SalesPrice>item.SalesPrice-5000 
                && a.SalesPrice<item.SalesPrice+5000
        ...
        public List<TbItem> GetByItemId(int id)
        {
            try
            {
                var item = context.TbItems.Where(a => a.ItemId == id && a.CurrentState == 1).ToList();
                return item;
            }
            catch
            {
        ...
        public List<VwItem> GetByCatId(int id)
        {
            try
            {
                var items = context.VwItems.Where(a => a.CategoryId == id && a.CurrentState == 1).ToList();
                return items;
            }
            catch
            {
